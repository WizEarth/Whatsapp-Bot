...
exports.smsg = (Izz, m, store) => {
  ...
  if (m.key) {
    ...
    m.sender = Izz.decodeJid(m.fromMe && Izz.user.id || m.participant || m.key.participant || m.chat || '');
    if (m.isGroup) m.participant = Izz.decodeJid(m.key.participant) || '';
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.sender = Izz.decodeJid(m.msg.contextInfo.participant);
      m.quoted.fromMe = m.quoted.sender === Izz.decodeJid(Izz.user.id);
      ...
      m.quoted.delete = () => Izz.sendMessage(m.quoted.chat, {
        delete: vM.key
      });
      m.quoted.copyNForward = (jid, forceForward = false, options = {}) => Izz.copyNForward(jid, vM, forceForward, options);
      m.quoted.download = () => Izz.downloadMediaMessage(m.quoted);
    }
  }
  if (m.msg.url) m.download = () => Izz.downloadMediaMessage(m.msg);
  ...
  m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? Izz.sendMedia(chatId, text, 'file', '', m, {
    ...options
  }) : Izz.sendText(chatId, text, m, {
    ...options
  });
  m.copy = () => exports.smsg(conn, M.fromObject(M.toObject(m)));
  m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => Izz.copyNForward(jid, m, forceForward, options);
  Izz.appenTextMessage = async (text, chatUpdate) => {
    ...
    messages.key.fromMe = areJidsSameUser(m.sender, Izz.user.id);
    ...
    Izz.ev.emit('messages.upsert', msg);
  };
  return m;
};
